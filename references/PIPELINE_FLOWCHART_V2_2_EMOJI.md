# 🔄 Pipeline Flowchart v2.2 — Constitution-First Multi-Agent Delivery with Enhanced Gemini Review
...
═══════════════════════════════════════════════════════════
  ♻️  Step 5.5 — Epoch Fallback + Enhanced Diagnosis (max 3 Epochs)
  Orchestrator: ☀️ main
═══════════════════════════════════════════════════════════

  Entry: Step4-R3 fail OR TF-3 fail

  1️⃣ spawn gemini /think high
     → 产出诊断 memo
     → 快速分析失败模式
     → 织梦群 + 监控群

  2️⃣ spawn review (swap gemini) /think medium
     → 复核诊断 memo
     → 检查是否遗漏关键信息
     → 输出：COMPLETE / INCOMPLETE
     → 交叉审核群 + 监控群

  3️⃣ spawn brainstorming /model opus /think high
     → 根因分析 + 回滚决策
     → 输入：诊断 memo + 复核结果 + epoch-history.json
     → 输出：回滚决策 + 理由 + 信心度
     → 灵感熔炉群 + 监控群

  4️⃣ spawn review (swap gpt) /think high（按需）
     → 仲裁回滚决策
     → 触发条件：
       • Epoch >= 2
       • 决策为回滚到 S1
...
**强化 Step 5.5 Epoch 诊断与仲裁：**

1. **增加 review/gemini 复核诊断**
   - gemini 产出诊断 memo 后
   - review/gemini 复核诊断，检查是否遗漏关键信息
   - 避免诊断不完整导致错误决策

2. **增加 review/gpt 仲裁回滚决策**
   - brainstorming 产出回滚决策后
   - review/gpt 仲裁决策（高风险场景）
